...
function hashNum(n) { let h = n; h = ((h >> 16) ^ h) * 0x45d9f3b; h = ((h >> 16) ^ h) * 0x45d9f3b; h = (h >> 16) ^ h; return Math.abs(h % 10000) / 10000; }

function getMinuteMultiplier(rarity, minuteOffset) {
  ...
  return +(0.5 + base * 2.5 + Math.sin(minute * 0.1) * 0.3).toFixed(1);
}
...
export const yamadaService = {
  getSessionStatus() {
    ...
    if (!session) { _yamadaCache = null; return { active: false, reason: '山田先生不在，请于 9:00-12:00 / 15:00-18:00 / 21:00-23:00 来访' }; }
    var now = new Date();
    var currentMinute = Math.floor(now.getTime() / 60000);
    if (_yamadaCache && _yamadaCache.minute === currentMinute && _yamadaCache.session === session.label) return _yamadaCache.status;
    var prices = {};
  ...
  getSellableBottles() {
    ...
    return bottleService.getInventory().filter(item => {
  ...
  isAvailable() { return !!getCurrentSession(); },
};
